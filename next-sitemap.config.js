...
module.exports = {
  siteUrl: "https://ai.tomokichidiary.com",
  ...
  robotsTxtOptions: {
    policies: [
      { userAgent: '*', allow: '/' },
      { userAgent: '*', disallow: ['/test/', '/my-plans/', '/plan/', '/auth/', '/admin/'] }
    ]
  },
  additionalPaths: async (config) => {
    ...
    const samplePlansPath = path.join(__dirname, 'src/lib/sample-plans.ts');
    const additionalPlansPath = path.join(__dirname, 'src/lib/additional-sample-plans.ts');
